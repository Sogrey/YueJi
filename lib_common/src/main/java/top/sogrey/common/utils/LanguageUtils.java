...
import top.sogrey.common.utils.ktx.SPKtxUtilsKt;
...
public class LanguageUtils {
    ...
    private static void applyLanguage(@NonNull final Locale locale,
                                      ...
                                      final boolean isFollowSystem) {
        if (isFollowSystem) {
            SPKtxUtilsKt.setSP(AppUtils.getApp(),KEY_LOCALE, VALUE_FOLLOW_SYSTEM);
        } else {
            String localLanguage = locale.getLanguage();
            String localCountry = locale.getCountry();
            SPKtxUtilsKt.setSP(AppUtils.getApp(),KEY_LOCALE, localLanguage + "$" + localCountry);
        }
        ...
        intent.setComponent(new ComponentName(AppUtils.getApp(), realActivityClassName));
        intent.addFlags(
    ...
    static void applyLanguage(@NonNull final Activity activity) {
        final String spLocale = (String) SPKtxUtilsKt.getSP(activity,KEY_LOCALE,"");
        ...
        String[] languageCountry = spLocale.split("\\$");
        if (languageCountry.length != 2) {
            LogKtUtils.Companion.e("LanguageUtils", "The string of " + spLocale + " is not in the correct format.");
            return;
        }

        Locale settingLocale = new Locale(languageCountry[0], languageCountry[1]);
